...
- **`Codex/windows-client`** — **Windows client only** (tracked by PR #20). Put Windows
...
(`Codex/4d-memory-rfc`, `Codex/mom1-access-control`, `Codex/terr-management-ui`):
```
Codex/<short-topic>        # e.g. Codex/markdown-memory-backend
```
`feat/<topic>` / `fix/<topic>` are also acceptable for conventional work.

## Process
1. Pick the correct base: Windows work → `Codex/windows-client`; everything else → `main`.
...
3. Create and switch:
   ...
   ```
4. When pushing the first time: `git push -u origin Codex/<short-topic>`.
