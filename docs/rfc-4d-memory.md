...
```
activation(e, now, ctx) = w_time(e.time, now, ctx)   # 该不该触发，权重多少
                        × align(e.aim, ctx)          # 目的是否对齐（0 则不激活）
                        × utility(e.use)             # 历史使用效用（越用越有用则升）
...
class Trigger:
    ...
    def weight(self, entry: "MemoryEntry", now: float, ctx: "ActivationContext") -> float:
...
class MemoryEntry:
    ...
    time: Trigger = field(default_factory=Trigger)
```
