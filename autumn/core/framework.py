...
class Autumn:
    ...
    Entry point that wires all components together from a config.
    Usage:
        async with Autumn(config) as autumn:
            result = await autumn.process(user_input)
    """

    def __init__(self, config: AutumnConfig):
        self.config = config
        self.plugins = PluginLoader()
        self._build(config)

    def _build(self, config: AutumnConfig) -> None:
        ...
        self.wp3 = WP3Mis(self.a3, self.mom3, wp2=self.wp2)
        self.wp1 = WP1Tot(self.a1, self.mom1, self.wp2, self.wp3)
