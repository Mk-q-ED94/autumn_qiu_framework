...
class WP2Tas(WorkspaceBase):
    ...
    async def stream(
        ...
        if tools or skills:
            ...
            # Token totals from the agent loop are discarded here — streaming runs
            # don't propagate per-stage trace info.
            result, _, _ = await self._run_with_agent(task_input, tools, skills, None, task_type)
